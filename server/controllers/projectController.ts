...
export const createProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prompt = `Professional advertisement photo.
    ...
    const imageUrl = genResponse.data.output;
    ...
    // const pollinationsUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?seed=${Math.floor(Math.random() * 1000000)}&nologo=true&model=flux`;
    ...
    // fal ai
    // const falResult = await axios.post(
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //   where: { id: projectId },
    //   data: {
    //     generatedVideo: project.generatedImage,
    //     isGenerating: false,
    //   },
    // });
    ...
    // return res.json({
    //   success: true,
    //   videoUrl: project.generatedImage,
    // });
    ...
    // new free video generation

    //const filePath = project.generatedImage;
    const imagePath = `videos/${Date.now()}.png`;
